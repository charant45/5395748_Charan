...
int* dynamicArray(int n, int queries_rows, int queries_columns, int** queries, int* result_count) {
    int** sequences = malloc(n * sizeof(int*));
    for (int i = 0; i < n; i++) {
    ...
    *result_count = 0;
    ...
    for (int i = 0; i < queries_rows; i++) {
        ...
        if (queryType == 1) {
            sequences[seqIndex] = realloc(sequences[seqIndex], (sizes[seqIndex] + 1) * sizeof(int));
            sequences[seqIndex][sizes[seqIndex]] = y;
            sizes[seqIndex]++;
        } else if (queryType == 2) {
            lastAnswer = sequences[seqIndex][y % sizes[seqIndex]];
